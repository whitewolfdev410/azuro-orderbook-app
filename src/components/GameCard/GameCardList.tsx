    ...
    <Link href={`/event/${gameId}`}>
      <div
        className={clsx(
          ...
          'grid grid-cols-5 auto-rows-auto gap-4'
        )}
      >
        <div className="col-start-1 row-start-2 flex flex-col flex-1">
          <Participant
            {...participants[0]}
            className="flex-row"
            size={'xs'}
        ...
        <div className="max-lg:hidden row-start-2 col-start-2 flex items-center justify-end">
          {markets[1]?.outcomeRows[0].length < 3 && markets[1]?.name}
        </div>
        <div className="max-lg:hidden row-start-1 col-start-3 flex justify-around items-center">
          {markets[1]?.outcomeRows[0].length < 3 && markets[1]?.outcomeRows[0].map((outcome, index) => (
            <div key={index}>{outcome.selectionName}</div>
          ))}
        </div>
        <div className="max-lg:hidden row-start-2 col-start-3 flex justify-around items-center">
          {markets[1]?.outcomeRows[0].length < 3 && markets[1]?.outcomeRows[0].map((outcome, index) => (
            <div>
              <OutcomeButton
                index={index}
        ...
        <div className="max-lg:hidden row-start-2 col-start-4 flex items-center justify-end">
          {markets[0]?.name}
        </div>
